...
| R4 | Every crop and unit tile is an illustration **somebody who draws for a living has looked at**. All 89 are now drawn rather than hatched — `scripts/illustrate.py`, flat shapes, family-tinted grounds, silhouettes chosen so the three greens and the three peppers are told apart by shape and not only by colour. What is still missing is the judgement of a person who has seen the actual crops in an actual market: whether a Nigerian farmer looking at the `ugu` tile says *ugu*. The gate is no longer "there is nothing to look at" but "nobody qualified has looked" | An illustrator, and somebody who has seen the crops and the measures | Phase 1 |
...
| R11 | The **capture thresholds are calibrated on real photographs**. `domain/diagnosis/framing.dart` decides whether a frame is worth photographing — too dark, too bright, too blurry, ready — and the arithmetic is exact and tested. The four numbers it compares against are not: `darkest = 0.18`, `brightest = 0.82`, `mostClipped = 0.10`, `leastDetail = 0.012` have never been measured against a photograph of a leaf, because there is none in this repository and no handset to take one with. Being too strict is the safe direction — a rejected frame costs two seconds, an accepted bad one costs an answer about a disease — so the calibration moves them to that side of whatever the real distribution turns out to be. See [ADR-0010](adr/0010-the-photograph-is-judged-before-it-is-taken.md) | A handset, and leaves in a field at more than one time of day | Phase 4 |
