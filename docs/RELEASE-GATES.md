...
| R5 | The bundled clips are **compressed**. 220 placeholder WAVs are already 29 MB, and the P0 vocabulary is a fraction of v1.0's. WAV was chosen so `audio-check` could open a clip and prove it is not silent; that gate has to survive the format change rather than be dropped with it | A decision on format, and a way to check a compressed clip is not silent | Phase 2 |
