...
| A recording kit for native speakers — script, prompts, a phone-recording guide | Blocks R1, and R1 blocks v1.0. Worth building the moment there is somebody to hand it to. |
...
| Replace or upgrade `flutter_timezone` | It applies the Kotlin Gradle Plugin, and the first Android build warned that **future Flutter versions will fail to build** because of it. Not broken today; a dependency with a known expiry date, on the platform the product can least afford to lose. |
